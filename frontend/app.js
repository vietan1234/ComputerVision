...
async function selectProfile(p) {
  ...
  // Display enrolled image if available (from first template)
  if (rawTemplates3.length > 0 && rawTemplates3[0].enrolled_image) {
    const img = rawTemplates3[0].enrolled_image;
    const mime = img.image_mime || 'image/bmp';
    $('#enrolledImg2').src = `data:${mime};base64,${img.image_b64}`;
    addLog('Hiển thị ảnh enrolled đã lưu', 'info', 'tab2');
  }
...
async function saveRaw3() {
  ...
  // Prepare enrolled_image from last captured bitmap
  let enrolled_image = null;
  if (lastBitmapBase64) {
    enrolled_image = {
      image_b64: lastBitmapBase64,
  ...
  addLog('Đang lưu hồ sơ với template + ảnh enrolled...', 'info', 'tab1');
