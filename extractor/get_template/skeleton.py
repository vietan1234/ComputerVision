...
# Binarization and Thinning - khôi phục bản ổn định
...
def _prune_spurs(skel: np.ndarray, iterations: int = 3) -> np.ndarray:
    """Loại bỏ spurs (nhánh ngắn) từ skeleton."""
    ...
    for _ in range(iterations):
        ...
        s = s & (~leaves)
    # Loại connected components nhỏ (isolated noise)
...
def binarize_and_thin(enhanced: np.ndarray):
    """
    Binarize với adaptive threshold và thinning.
    
    Đây là bản ổn định - sử dụng adaptive threshold trên toàn ảnh.
    """
    # Adaptive threshold - tốt cho fingerprint vì tự động điều chỉnh local
    ...
    # Invert: sau adaptive, ridges thường là dark (0) -> cần invert để ridges = 255
    bin_img = 255 - bin_img
    
    # Median blur để giảm noise nhỏ
    ...
    # Prune spurs
    skel = _prune_spurs(skel, iterations=3)
